...
@Service
public class GetfundtransfersByIdRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(GetfundtransfersByIdRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.getfundtransfersbyid.hosturl}")
	private String url;
	
	@Value("${spring.api.getfundtransfersbyid.context-path}")
	private String contextPath;

	@Value("${spring.api.getfundtransfersbyid.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  GetfundtransfersByIdRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ GetfundtransfersByIdRestProcessor.class.getName());
