...
@Service
public class GetfundtransfersRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(GetfundtransfersRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.getfundtransfers.hosturl}")
	private String url;
	
	@Value("${spring.api.getfundtransfers.context-path}")
	private String contextPath;

	@Value("${spring.api.getfundtransfers.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  GetfundtransfersRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ GetfundtransfersRestProcessor.class.getName());
